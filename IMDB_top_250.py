...
#IMDB started from 1990

df = pd.read_csv('IMDB Top 250 Movies.csv')

# check any null in the whole dataframe => False, the usability for this data is 10.0
...
sns.histplot(data=dfYear, x="year",binwidth=5,kde=True, shrink=0.8).set(title='Number of Movies in IMDB Top 250 by Year',xlabel='Year',ylabel='Number of Movies')
...
topMovies = df.nlargest(3,['rating'])
topMovies = topMovies[['name','rating']]
...
dfGenre = pd.melt(df, id_vars=['name', 'rating','box_office'], value_vars=['genre1', 'genre2', 'genre3'], var_name='genre', value_name='type')
...
# df group by Genre
dfGenre.loc[dfGenre['box_office'].str.endswith(")"), 'box_office'] = dfGenre['box_office'].str.replace(r' \(estimated\)',"", regex=True)
...
sns.barplot(groupbyGenredf,y='type',x='box_office').set(title='Mean Box Office by Movie Type',xlabel='Mean Box Office')
...
# 整理版 分級
# print(df['certificate'].unique())
dfCertificate = df.drop(df[(df['certificate'] == 'Not Rated') | (df['certificate'] == 'Not Available')| (df['certificate'] == 'Unrated')].index)
colors = ['#f94144', '#f9844a', '#f9c74f', '#90be6d']
sns.countplot(data=dfCertificate, x="certificate",order=dfCertificate["certificate"].value_counts().index, palette=colors).set(title='Certificate Count',xlabel='Certificate')
...
# 注意有些時間沒有分的 一定要補上0 否則none type 這裡會無法做加總
...
# brackets 在字串中會被視為是符號 所以要加上escape sequence "\" 在該符號前面
df.loc[df['box_office'].str.endswith(")"), 'box_office'] = df['box_office'].str.replace(r' \(estimated\)',"", regex=True)
...
dfGorssProfitdf2.loc[:,'GrossProfit'] = (dfGorssProfitdf['box_office'] - dfGorssProfitdf['budget'])/dfGorssProfitdf['box_office']
